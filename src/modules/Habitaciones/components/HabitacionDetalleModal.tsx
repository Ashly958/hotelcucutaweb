...
export function HabitacionDetalleModal({
  ...
  const [acompanantes, setAcompanantes] = useState<Array<{
    nombres: string;
  ...
  const handleEjecutarCheckIn = async (e: React.FormEvent) => {
    ...
    await onCheckIn({
      ...
      noches,
      abonoEfectivo: abono,
      acompanantes: acompanantes.filter(ac => ac.nombres.trim() !== ''),
    });
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-xs flex items-center justify-center p-4">
      <div className="bg-white rounded-3xl max-w-lg w-full shadow-xl border border-slate-200/80 overflow-hidden animate-fadeIn">
        {/* Header Modal - Minimalista & Luminoso */}
        <div className="bg-white border-b border-slate-100 p-5 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="h-10 w-10 bg-red-50 text-red-700 border border-red-200/60 rounded-xl flex items-center justify-center font-display font-bold text-xl shadow-soft-xs">
              {habitacion.numero}
            </div>
            <div>
              <h3 className="font-display font-bold text-lg text-slate-900 leading-tight tracking-tight">
          ...
          <button
            onClick={onClose}
            className="p-2 rounded-xl text-slate-400 hover:text-slate-700 hover:bg-slate-50 border border-transparent hover:border-slate-200/60 transition-colors"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="p-6 space-y-5 text-xs text-slate-700">
          ...
          <div className="grid grid-cols-3 gap-3 bg-slate-50/70 p-3.5 rounded-2xl border border-slate-200/60 text-center">
            <div>
              <span className="text-slate-400 block text-[10px] uppercase">Climatización</span>
              <strong className="text-slate-900 font-semibold">
                {habitacion.tieneAire ? 'Aire Acondicionado' : 'Ventilador'}
              </strong>
            </div>
            <div>
              <span className="text-slate-400 block text-[10px] uppercase">Capacidad Máx</span>
              <strong className="text-slate-900 font-semibold">{habitacion.capacidadMax} personas</strong>
            </div>
            <div>
              <span className="text-slate-400 block text-[10px] uppercase">Tarifa</span>
            ...
            <div className="space-y-4">
              <div className="bg-slate-50/70 border border-slate-200/70 p-4 rounded-2xl space-y-3">
                <div className="flex items-center justify-between border-b border-slate-200/60 pb-2">
                  ...
                  <span className="text-[10px] bg-red-50 text-red-700 border border-red-200/60 px-2 py-0.5 rounded-full font-mono font-medium">
                    Ocupada
                  </span>
                </div>
              ...
              <div className="flex items-center justify-between pt-2">
                <span className="text-[11px] text-slate-400 italic">
                  * Salida habitual: 1:00 PM (RN-008).
                </span>
                <Button
                  variant="primary"
                  size="md"
          ...
          {habitacion.estado === 'limpieza' && (
            <div className="p-5 bg-slate-50/70 border border-slate-200/70 rounded-2xl space-y-3 text-center">
          ...
          {habitacion.estado === 'mantenimiento' && (
            <div className="p-5 bg-amber-50/60 border border-amber-200/60 rounded-2xl space-y-3">
              ...
              <p className="text-amber-800 text-[11px]">
                {habitacion.observaciones || 'Revisión técnica o preventiva en curso.'}
              </p>
          ...
          {habitacion.estado === 'disponible' && !modoCheckIn && (
            <div className="p-5 bg-emerald-50/50 border border-emerald-200/60 rounded-2xl text-center space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2.5">
                <div className="col-span-2">
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 text-xs bg-white border border-slate-200/80 rounded-xl outline-none focus:border-red-500 focus:ring-2 focus:ring-red-500/10 shadow-2xs"
                  />
                </div>
                ...
                <div>
                  ...
                  <div className="flex gap-1.5">
                    <select
                      ...
                      className="w-16 px-2 py-2 text-xs bg-white border border-slate-200/80 rounded-xl outline-none focus:border-red-500 shadow-2xs"
                    >
                    ...
                    <input
                      ...
                      className="w-full px-3 py-2 text-xs bg-white border border-slate-200/80 rounded-xl outline-none focus:border-red-500 focus:ring-2 focus:ring-red-500/10 shadow-2xs font-mono"
                    />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 text-xs bg-white border border-slate-200/80 rounded-xl outline-none focus:border-red-500 shadow-2xs"
                  />
                </div>
                ...
                <div className="flex gap-2">
                  <div className="w-1/2">
                    <label className="block text-[11px] font-medium text-slate-600 mb-1">
                      Personas (Máx {habitacion.capacidadMax})
                    </label>
                    <select
                      value={pax}
                      onChange={(e) => {
                        ...
                        setPax(newPax);
                        const neededAc = Math.max(0, newPax - 1);
                        setAcompanantes((prev) => {
                          const updated = [...prev];
                          while (updated.length < neededAc) {
                            updated.push({
                              nombres: '',
                              tipo_documento: 'CC',
                              numero_documento: '',
                          ...
                          return updated.slice(0, neededAc);
                        });
                      }}
                      className="w-full px-2 py-2 text-xs bg-white border border-slate-200/80 rounded-xl outline-none focus:border-red-500 shadow-2xs"
                    >
                      {Array.from({ length: habitacion.capacidadMax }, (_, i) => i + 1).map((n) => (
                  ...
                  <div className="w-1/2">
                    ...
                    <input
                      ...
                      className="w-full px-3 py-2 text-xs bg-white border border-slate-200/80 rounded-xl font-mono outline-none focus:border-red-500 shadow-2xs"
                    />
                  </div>
                </div>

                {/* Acompañantes cuando pax > 1 */}
                {pax > 1 && acompanantes.length > 0 && (
                  <div className="p-3 bg-slate-50/80 border border-slate-200/80 rounded-2xl space-y-2">
                    <span className="text-[11px] font-semibold text-slate-800 block">
                      Acompañantes adicionales ({acompanantes.length})
                    </span>
                      ...
                      <div key={idx} className="grid grid-cols-5 gap-2">
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            updated[idx] = { ...updated[idx], nombres: e.target.value };
                            setAcompanantes(updated);
                          }}
                          className="col-span-3 px-2.5 py-1.5 text-xs bg-white border border-slate-200 rounded-lg outline-none focus:border-red-500"
                        />
                        <input
                          type="text"
                          placeholder="Documento"
                          value={ac.numero_documento}
                          onChange={(e) => {
                            const updated = [...acompanantes];
                            updated[idx] = { ...updated[idx], numero_documento: e.target.value };
                            setAcompanantes(updated);
                          }}
                          className="col-span-2 px-2 py-1.5 text-xs bg-white border border-slate-200 rounded-lg font-mono outline-none focus:border-red-500"
                        />
                      </div>
                    ))}
                  </div>
